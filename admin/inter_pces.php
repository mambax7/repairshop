...
switch ($op) {
    ...
    case 'create':
        ...
        while (isset($_POST['idkey_' . $idconst])) {
            ...
            $sqli = 'UPDATE '
                    . $xoopsDB->prefix('garage_inter_temp')
                    . " SET observation='"
                    . $observation
                    . "', hmeca_t1='"
                    . $maj_hmeca_t1
                    ...
                    . "' WHERE id="
                    . $key;
            $xoopsDB->queryF($sqli) || exit('Erreur requete : ' . $sqli . '<br>');
            ++$idconst;
        }
...
if (empty($op)) {
    ...
    while ((list($id2, $id_vehic, $kilometrage, $date_debut, $date_fin, $delai, $id_inter_recurrente, $description, $observation, $date_devis, $date_acceptation, $montant, $acompte_verse, $solde, $hmeca_t1, $hmeca_t2, $hmeca_t3, $hcarro_t1, $hcarro_t2, $hcarro_t3, $tmeca_t1, $tmeca_t2, $tmeca_t3, $tcarro_t1, $tcarro_t2, $tcarro_t3, $remise_meca, $remise_caro, $remise_forfait, $numero_devis, $numero_facture, $id_vehicule, $immat, $id_marque, $gamme, $modele_version, $id_proprietaire, $civilite, $nom, $prenom, $teldom, $telport, $marque) = $xoopsDB->fetchRow($requete))
        ...
        $form->insertBreak(_AM_VEHICULE_INTERVENTION . '<div class="center;"><br>' . $etrep . '</div>', 'head');
        ...
        $form->insertBreak('<h3><div class="center;">' . _AM_INTER_PIECES . '</div></h3>', 'head');
        ...
        while ((list($idkey, $id_piece, $designation, $id_fournisseur, $quantite, $tarif_client, $remise_pieces) = $xoopsDB->fetchRow($reqp)) !== false) {
            ...
            if (null !== $id_piece) {
                ...
                while ((list($designation, $tarif_client_std, $nom_fournisseur) = $xoopsDB->fetchrow($reqpce)) !== false) {
                    ...
                    $pieces_det .= "<td><a href='inter_pces.php?op=delete&id=" . $idkey . '&id_inter=' . $id_inter . "'><img src='../assets/images/cancel.png' alt='" . _AM_DELETE . "' title='" . _AM_DELETE . "'></a></td></tr>";
                }
            }
            //piece libre
            if (null === $id_piece) {
        ...
        while ((list($idkey, $idinter, $id_forfait, $designation_ff, $ref_fournisseur, $quantite_ff, $tarif_client) = $xoopsDB->fetchRow($reqf)) !== false) {
            ...
            $forfait .= "<td><a href='inter_pces.php?op=delete_forf&id=" . $idkey . '&id_inter=' . $id_inter . "'><img src='../assets/images/cancel.png' alt='" . _AM_DELETE . "' title='" . _AM_DELETE . "'></a></td></tr>";

            $montant_forfait_brut = $montant_brut_forfait + $tarif_client;
            $remise_forf          += $remise_forfait;
        ...
        $forfait .= "<td style='border: 1px solid #D2A901;' align=right>"
                    . $montant_forfait_brut
                    . ' '
                    . $xoopsModuleConfig['money']
                    . "<br><input type='text' name='remise_forfait' size='10' maxlength='15' value='"
                    . $remise_forfait
                    . "' > "
                    . $xoopsModuleConfig['money']
                    . '<br>'
